...
import { ArrowLeft, CheckCircle2, Clock3, Copy, QrCode, ShieldCheck, Sparkles } from "lucide-react";
    ...
    <main className="min-h-[100dvh] bg-[#080909] px-4 pb-8 text-white sm:px-6">
      <header className="mx-auto flex max-w-xl items-center gap-3 border-b border-white/10 py-4">
        <Link
          ...
          className="grid size-11 shrink-0 place-items-center rounded-full border border-white/10 bg-white/[.04] transition hover:border-[#ffc400]/50"
        >
      ...
      <section className="mx-auto max-w-xl py-8 text-center sm:py-12">
        <div className="mx-auto grid size-14 place-items-center rounded-2xl border border-[#ffc400]/20 bg-[#ffc400]/10 text-[#ffc400]">
        ...
        <div className="mx-auto mt-7 max-w-[300px] rounded-3xl border border-white/10 bg-white p-5 shadow-[0_12px_50px_rgba(0,0,0,.25)]">
          <div className="flex aspect-square flex-col items-center justify-center rounded-2xl border-2 border-dashed border-zinc-300 bg-zinc-50 px-5 text-center text-zinc-900">
            <div className="grid size-16 place-items-center rounded-2xl bg-zinc-200 text-zinc-700">
        ...
        <button
          type="button"
          disabled
          className="mt-3 flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[.04] px-4 font-bold text-white/35"
          title="Disponível após integrar o provedor de pagamento"
        >
          <Copy className="size-4" />
          Código PIX indisponível nesta prévia
        </button>

        <div className="mt-6 flex items-center justify-center gap-2 rounded-xl border border-[#ffc400]/15 bg-[#ffc400]/[.06] px-3 py-3 text-sm font-semibold text-[#ffc400]">
          <Clock3 className="size-4 shrink-0" />
          Aguardando integração do pagamento
        </div>
        ...
        <div className="mt-7 grid gap-3">
          <Link to="/" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#ffc400] px-5 font-black text-black transition hover:bg-yellow-300">
