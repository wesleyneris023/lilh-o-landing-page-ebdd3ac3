...
import {
  MapPin, Clock, Phone, Instagram, ShoppingBag, Plus, Minus, X, Search,
  Trash2, CheckCircle2, Utensils, Play,
} from "lucide-react";
...
function Header({ quantidade, abrirSacola }: { quantidade: number; abrirSacola: () => void }) {
  return <header className="sticky top-0 z-40 border-b border-white/10 bg-[#11110f]/95 backdrop-blur-md">
    <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6">
      <a href="#inicio" className="flex items-center gap-2 font-black tracking-tight text-white"><span className="grid size-10 place-items-center rounded-full border-2 border-[#ffc400] bg-[#201b0c] text-sm text-[#ffc400]">L!</span><span className="text-2xl">Lilhão<span className="text-[#ffc400]">.</span></span></a>
      <nav className="hidden items-center gap-7 text-sm font-semibold text-white/70 md:flex"><a href="#inicio" className="hover:text-[#ffc400]">Início</a><a href="#cardapio" className="hover:text-[#ffc400]">Cardápio</a><a href="#contato" className="hover:text-[#ffc400]">Contato</a></nav>
      <button onClick={abrirSacola} className="relative inline-flex min-h-11 items-center gap-2 rounded-full bg-[#ffc400] px-4 py-2 text-sm font-extrabold text-black transition hover:bg-[#ffda4d]" aria-label="Abrir sacola"><ShoppingBag className="size-5"/><span className="hidden sm:inline">Minha sacola</span><span className="grid size-6 place-items-center rounded-full bg-black text-xs text-[#ffc400]">{quantidade}</span></button>
    </div>
  </header>;
}
...
function Index() {
  ...
  const filtrados = useMemo(() => produtos.filter((p) => (categoria === "Todos" || p.categoria === categoria) && `${p.nome} ${p.descricao}`.toLowerCase().includes(busca.toLowerCase())), [categoria, busca]);
  ...
  function adicionar(produto: Produto) {
    setSacola((atual) => { const existe = atual.find((item) => item.id === produto.id); return existe ? atual.map((item) => item.id === produto.id ? { ...item, quantidade: item.quantidade + 1 } : item) : [...atual, { ...produto, quantidade: 1 }]; });
    setPedidoMontado(false);
  }
  function alterarQuantidade(id: string, delta: number) {
    setSacola((atual) => atual.map((item) => item.id === id ? { ...item, quantidade: item.quantidade + delta } : item).filter((item) => item.quantidade > 0));
    setPedidoMontado(false);
  }
  function montarPedido(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!sacola.length || !nomeCliente.trim() || !bairro.trim()) return;
    setPedidoMontado(true); setCheckout(false);
  }

  return <div className="min-h-screen bg-[#10100e] text-white">
    <Header quantidade={quantidade} abrirSacola={() => setSacolaAberta(true)} />
    <main>
      <section id="inicio" className="relative isolate flex min-h-[82svh] items-center justify-center overflow-hidden border-b border-white/10 bg-black text-center sm:min-h-[88svh]">
        <video className="absolute inset-0 -z-20 h-full w-full object-cover" autoPlay muted loop playsInline preload="metadata" poster={heroBurger} aria-hidden="true">
          <source src="https://videos.pexels.com/video-files/4253311/4253311-hd_1920_1080_25fps.mp4" type="video/mp4" />
        </video>
        <img src={heroBurger} alt="" className="absolute inset-0 -z-30 h-full w-full object-cover" />
        <div className="absolute inset-0 -z-10 bg-black/65" />
        <div className="relative mx-auto flex w-full max-w-5xl flex-col items-center px-5 py-20 sm:px-8 sm:py-24">
          <span className="inline-flex items-center gap-2 rounded-full border border-[#ffc400]/50 bg-black/45 px-4 py-2 text-xs font-black uppercase tracking-[.2em] text-[#ffc400] backdrop-blur-sm"><Utensils className="size-4"/> Lanchonete do Lilhão</span>
          <h1 className="mt-7 text-6xl font-black uppercase leading-[.88] tracking-tight drop-shadow-2xl sm:text-8xl md:text-9xl">Lilhão<span className="text-[#ffc400]">.</span></h1>
          <p className="mt-6 max-w-2xl text-lg font-medium leading-relaxed text-white/90 drop-shadow sm:text-2xl">Hambúrgueres, combos e aquele sabor que conquista.</p>
          <a href="#cardapio" className="mt-9 inline-flex min-h-14 items-center justify-center gap-3 rounded-full bg-[#ffc400] px-10 text-base font-black uppercase tracking-wide text-black shadow-[0_8px_35px_rgba(255,196,0,.25)] transition hover:scale-[1.03] hover:bg-[#ffda4d]">Ver cardápio <span aria-hidden="true">↓</span></a>
          <div className="mt-7 flex items-center gap-2 text-xs font-bold uppercase tracking-[.16em] text-white/65"><Play className="size-3 fill-current"/> Sabor que dá vontade</div>
        </div>
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-[#10100e] to-transparent" />
      </section>

      <section id="cardapio" className="mx-auto max-w-7xl scroll-mt-20 px-4 py-14 sm:px-6 sm:py-20">
        <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-xs font-black uppercase tracking-[.2em] text-[#ffc400]">Escolha o seu</p><h2 className="mt-2 text-4xl font-black uppercase sm:text-5xl">Nosso cardápio</h2><p className="mt-3 max-w-xl text-sm text-white/60">Selecione seus favoritos e adicione à sacola. Os valores abaixo seguem o cardápio enviado.</p></div>
        ...
        <div className="mt-7 flex gap-2 overflow-x-auto pb-2">{categorias.map((item) => <button key={item} onClick={() => setCategoria(item)} className={`shrink-0 rounded-full px-5 py-2.5 text-sm font-extrabold transition ${categoria === item ? "bg-[#ffc400] text-black" : "border border-white/15 bg-white/[.03] text-white/70 hover:border-[#ffc400]/50 hover:text-white"}`}>{item}</button>)}</div>
        <div className="mt-7 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">{filtrados.map((p) => { const noCarrinho = sacola.find((item) => item.id === p.id)?.quantidade ?? 0; return <article key={p.id} className="group flex min-w-0 flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#191916] transition hover:border-[#ffc400]/40">
      ...
      <section id="contato" className="border-t border-white/10 bg-[#171714]"><div className="mx-auto max-w-7xl px-4 py-14 sm:px-6"><div className="text-center"><p className="text-xs font-black uppercase tracking-[.2em] text-[#ffc400]">Estamos por aqui</p><h2 className="mt-2 text-3xl font-black uppercase sm:text-4xl">Lilhão, sabor que conquista</h2></div><div className="mx-auto mt-8 grid max-w-4xl gap-4 sm:grid-cols-3"><div className="rounded-2xl border border-white/10 bg-white/[.03] p-5 text-center"><Clock className="mx-auto size-6 text-[#ffc400]"/><h3 className="mt-3 font-extrabold">Horários</h3><p className="mt-1 text-sm text-white/55">Consulte a disponibilidade do dia</p></div><div className="rounded-2xl border border-white/10 bg-white/[.03] p-5 text-center"><Phone className="mx-auto size-6 text-[#ffc400]"/><h3 className="mt-3 font-extrabold">Pedidos</h3><p className="mt-1 text-sm text-white/55">(96) 9911-2135</p></div><div className="rounded-2xl border border-white/10 bg-white/[.03] p-5 text-center"><MapPin className="mx-auto size-6 text-[#ffc400]"/><h3 className="mt-3 font-extrabold">Entrega</h3><p className="mt-1 text-sm text-white/55">Consulte a taxa para o seu bairro</p></div></div></div></section>
    </main>
    ...
    {sacolaAberta && <div className="fixed inset-0 z-50 flex justify-end bg-black/70" onClick={() => setSacolaAberta(false)}><aside role="dialog" aria-modal="true" aria-label="Sua sacola" onClick={(e) => e.stopPropagation()} className="flex h-full w-full max-w-md flex-col border-l border-white/10 bg-[#151512] shadow-2xl">
      <div className="flex items-center justify-between border-b border-white/10 p-5"><div><p className="text-xs font-black uppercase tracking-widest text-[#ffc400]">Seu pedido</p><h2 className="mt-1 text-2xl font-black">Minha sacola <span className="text-white/40">({quantidade})</span></h2></div><button onClick={() => setSacolaAberta(false)} aria-label="Fechar sacola" className="grid size-10 place-items-center rounded-full border border-white/15 hover:bg-white/10"><X className="size-5"/></button></div>
